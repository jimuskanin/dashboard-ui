    ...
    useEffect(() => {
        ...
        const timeline = gsap.timeline({
            scrollTrigger: {
                ...
                onUpdate: (self) => {
                    if (self.progress > 0) { // Only log after the scroll trigger is activated
                        console.log(self.progress);  // Log scroll progress for debugging
                    }
                }
            },
        });
        ...
        timeline
            .to(leftCurtainRef.current, { x: "-100%", ease: "none" })
            .to(rightCurtainRef.current, { x: "100%", ease: "none" })
            .to(contentAboveRef.current, { opacity: 0, ease: "none" });
